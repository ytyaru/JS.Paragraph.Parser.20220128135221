class ParagraphOutput extends ParseOutput {
    ...
    parse(text) {
        ...
        for (let p of ps) {
            ...
            /*
            console.log(spans)
            //spans = spans.filter(v => !v.startsWith('<h'))
            console.log(spans)
            const t = (spans.length < 2) ? `${spans[0]}` : `${spans.map(span=>ElementString.get('span', span)).join('<br>')}`
            if (0 < t.length) { html.push(ElementString.get('p', t)); }
//            html.push((spans.length < 2) ? `<p>${spans[0]}</p>` : `<p>${spans.map(span=>ElementString.get('span', span)).join('<br>')}</p>`);
    ...
    _getTextContents(spans) {
        if (1 === spans.length) {
            console.log(spans);
            return (spans[0].startsWith('<h')) ? spans[0] : ElementString.get('p', spans[0]);
        } else {
            //spans = spans.filter(v => v)
            console.log(spans);
